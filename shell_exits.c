 ...
 * @data: Structure contains potential argu. used to maintain
 * constant function prototype.
 * Return: exits with a given exits status
 * (0) if data->argu[0] != "exit"
 */
int exit_shell(data_d *data)
{
	int exit_status;

	if (data->arguments[1]) /* if there will be an exit argument */
	{
	...
	if (!data->arguments[1])
	{
		...
		if (!target_directory)
			change_directory_result = /* TODO: what should this be? */
				change_directory_function((target_directory = get_env_vari(data, "PWD=")) ? target_directory : "/");
		else
			change_directory_result = change_directory_function(target_directory);
	}
	else if (custom_strcmp(data->arguments[1], "-") == 0)
	{
		if (!get_env_vari(data, "OLDPWD="))
		{
			print_string(current_directory);
			print_char('\n');
			return (1);
		}
		print_string(get_env_vari(data, "OLDPWD="));
		print_char('\n');
		change_directory_result = /* TODO: what should this be? */
			change_directory_function((target_directory = get_env_vari(data, "OLDPWD=")) ? target_directory : "/");
	}
	else
